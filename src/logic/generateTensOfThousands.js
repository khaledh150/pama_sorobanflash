// ─────────────────────────────────────────────────────────────────────────────
// generateTensOfThousands.js — 5-digit Generator
// ─────────────────────────────────────────────────────────────────────────────
...
export function generateTensOfThousands(numSets, rows, mode) {
  const sets = [];
  for (let s = 0; s < numSets; s++) {
    let retryCount = 0, setGenerated = false;
    while (!setGenerated && retryCount < 50) {
      retryCount++;
      let cTThous = 0, cThous = 0, cHund = 0, cTens = 0, cUnits = 0;
      const currentSet = [];
      let validSet = true;
      let lTThous = 0, lThous = 0, lHund = 0, lTens = 0, lUnits = 0;

      for (let r = 0; r < rows; r++) {
        const canPlus = getValidMoves(cTThous, 1).length && getValidMoves(cThous, 1).length &&
                        getValidMoves(cHund, 1).length && getValidMoves(cTens, 1).length && getValidMoves(cUnits, 1).length;
        const canMinus = getValidMoves(cTThous, -1).length && getValidMoves(cThous, -1).length &&
                         getValidMoves(cHund, -1).length && getValidMoves(cTens, -1).length && getValidMoves(cUnits, -1).length;
        ...
        const fullMove = (mTThous * 10000) + (mThous * 1000) + (mHund * 100) + (mTens * 10) + mUnits;
        currentSet.push(fullMove);
        cTThous += mTThous; cThous += mThous; cHund += mHund; cTens += mTens; cUnits += mUnits;
        lTThous = mTThous; lThous = mThous; lHund = mHund; lTens = mTens; lUnits = mUnits;
      }
      if (validSet) {
         sets.push({ numbers: currentSet, answer: (cTThous * 10000) + (cThous * 1000) + (cHund * 100) + (cTens * 10) + cUnits });
         setGenerated = true;
      }
    }
  }
